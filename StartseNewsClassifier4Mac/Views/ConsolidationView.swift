...
//    let newsForClassification = ConsolidationViewModel()
            ...
            VStack {
                HStack {
                    ...
                    VStack {
                        ...
                        , label: {Text("Words").frame(width:80)})
                        Button(action: {}, label: {Text("Apagar").frame(width: 80)})
                        Button(action: {}, label: {Text("Termos").frame(width: 80)})
                        Spacer()
                    }.padding()
                }
            }
...
struct ConsolidationView_Previews: PreviewProvider {
    static var previews: some View {
        ConsolidationView(newsList: [NewsViewModel(newsModel: NewsModel(news_id: UUID().uuidString.uppercased(), title: "Conheça a Mission Barns, startup que está criando carne em laboratório", subtitle: "Startup foi criada em 2018, na universidade de Berkeley (EUA) e cultivo de origem animal para a produção de carne", link: "http://www.cin.ufpe.br", text: "Texto da notícia a ser classificada", links: [], links_text: []))])
    }
}
